...
public class Ground {
	...
	/* 存放自设石头的二维数组 */
	private boolean diyRocks[][] = new boolean[Global.WIDTH][Global.HEIGHT];
	 ...
	 *            格子坐标 y
	 */
	public boolean getDiyRock(int x, int y) {
		return diyRocks[x][y];
	}
	/**
	 * 自设格子状态
	 ...
	 *            格子坐标 y
	 */
	public void addDiyRock(int x, int y,boolean b) {
		diyRocks[x][y] = b;
	}
	//赋值
	public void diyStart() {
		for (int i = 0; i < rocks.length; i++) {
            for (int j = 0; j < rocks[0].length; j++) {
                rocks[i][j] = diyRocks[i][j];
            }
        }
	}
	 ...
	 * @return
	 */
	public boolean isDragonEatRock(Dragon dragon) {
		return rocks[dragon.getHead().x][dragon.getHead().y];
	}
